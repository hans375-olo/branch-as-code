...
## Quickstart (from a bare Ubuntu/Debian machine or VM)

```bash
# 0. Install the toolchain (containerlab's setup script also installs
#    Docker itself if it's missing)
sudo apt update && sudo apt install -y git make ansible python3-jmespath
curl -sL https://containerlab.dev/setup | sudo -E bash -s all
sudo usermod -aG docker $USER   # then log out & back in

# 1. Get the repo
git clone https://github.com/hans375-olo/branch-as-code.git
cd branch-as-code

# 2. Import the cEOS image (one-time, free arista.com account — see below).
#    docker import accepts .tar / .tar.gz / .tar.xz alike.
docker import cEOS64-lab-4.33.1F.tar ceos:4.33.1F

# 3. Bring up the reference branch — comes up fully configured
#    (baseline configs in configs/ are bind-mounted)
make lab-up

# 4. Render golden configs from the SoT and deploy them
cd ansible && ansible-galaxy collection install -r requirements.yml && cd ..
make render && make deploy

# 5. Prove it: LLDP, VLANs, BGP state assertion, end-to-end pings
make verify

# 6. Add the real SoT (optional — lab works fine without it)
...
pip install --break-system-packages pynetbox   # or use a venv
make populate          # builds the entire branch-001 data model
make render-netbox     # same templates, data now from NetBox
```

No `make`? Every target is a one-liner — read the Makefile and run the
commands directly.
...
| Switches | `ceos:4.33.1F` | 2 | ~1.5 GB | 3.0 GB |
...
(`ghcr.io/nokia/srlinux`, free to pull, ~1 GB) — only `templates/eos.j2`
...
1. Register at arista.com → *Software Downloads* → cEOS-lab → download
   `cEOS64-lab-4.33.1F.tar`
2. `docker import cEOS64-lab-4.33.1F.tar ceos:4.33.1F`

The download is a plain `.tar` (earlier cEOS releases shipped as `.tar.xz`) —
`docker import` handles both transparently. The important thing is that the
tag (`ceos:4.33.1F`) matches the `image:` lines in
`containerlab/branch-001.clab.yml`.
